...
import (
	...
	"github.com/aigccoin/go-aigccoin/crypto/ecies"
...
func (h *sessionState) readFrame(conn io.Reader) ([]byte, error) {
	...
	if !bytes.Equal(header[3:][:3],zeroHeader) {
		//fmt.Println("readFrame !bytes.Equal(header[3:][:3],zeroHeader):",header[3:][:3])
		return nil, errors.New("bad frame zeroHeader")
	}
...
var (
	...
	zeroHeader = []byte{0xE5, 0x60, 0x60}
