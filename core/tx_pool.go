...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if pool.currentMaxGas < tx.Gas() {
		err2 := fmt.Errorf("tx_pool %w: have %d, pool.currentMaxGas %d", ErrGasLimit, tx.Gas() , pool.currentMaxGas )
		log.Info(err2.Error())
		return ErrGasLimit
	}
	...
	if tx.Gas() < intrGas {
		err2 := fmt.Errorf("tx_pool %w: have %d, want %d", ErrIntrinsicGas, tx.Gas() , intrGas)
		log.Info(err2.Error())
