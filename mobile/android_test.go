...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.aigccoin", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-aigccoin issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.aigccoin", "github.com/aigccoin/go-aigccoin/mobile")
